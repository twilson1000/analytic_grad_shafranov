...
We can also find the volume averaged plasma pressure $\left<p\right> = \int p dV / \int dV$

$$ \left<p\right> = \frac{\int p(R, Z) 2\pi R dR dZ}{\int 2 \pi R dR dZ} = \frac{2 \pi R_0^3 \int p(x, y) x dx dy}{2 \pi R_0^3 \int x dx dy} = \frac{1}{V} \int p(x, y) x dx dy $$

where we have introduced the normalised volume $V = \int x dx dy$. Substituting in the expression for $p(x, y)$ from above

$$ \left<p\right> = \frac{\Psi_0^2}{\mu_0 R_0^4} (1 + A) \int \psi(x, y) x dx dy = \frac{\Psi_0^2}{\mu_0 R_0^4} (1 + A) I_2 $$
$$ I_2 = \int \psi(x, y) x dx dy $$

The poloidal beta is defined as $\beta_p = 2 \mu_0 \left<p\right> / \bar{B}_p^2 $ where $\bar{B} = \mu_0 I / R_0 C_p$ is the average poloidal magnetic field on the boundary $\psi = 0$ and $C_p$ is the plasma cross section circumference normalised by $R_0$. Substituting in the above expressions
